...
| 函数 | 使用场景 | 使用位置 |
|------|---------|---------|
| `ProcessBreak` | 检测到无法继续的条件，立即停止后续所有处理，自动跳转到epilog | Prolog（参数校验、文件检查） |
| `ProcessQuit()` | 在所有处理完成后根据累计错误决定是否标记失败，退出进程，数据不回滚 | Epilog（最终错误检查） |
| `ProcessError` | 一般性错误抛出不常用，结束进程，数据回归，通常用以上两种 | 按需使用 |
