...
- **cc-workon（spec-driven 工作流）**: 所有 TM1 MCP 写入工具受 `hooks/spec_gate.py` 拦截。开发前必须先注册：`: cc-workon models/<model_summary>` 或 `: cc-workon processes/<process_name>`。注册后需完成 spec 审查（创建 `.reviewed` 标记）才能调用写入工具。临时操作经用户确认可创建 `.spec_bypass`。每轮对话结束绑定自动清除，新一轮需重新注册。只读 MCP 工具不受限制。
