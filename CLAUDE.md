...
- **tm1_connector.py** — `TM1Manager` class: connection lifecycle, all TM1 read/write operations.
...
- **cc-workon（spec-driven 工作流必须）**: 本项目使用 spec-driven 工作流，所有 TM1 MCP 写入/破坏性工具都受 hook 拦截（`hooks/spec_gate.py`）。**在开始处理任何 TM1 项目开发前，必须先注册当前工作项目**，执行以下命令（注意：命令前需要加英文冒号）：
  ```
  : cc-workon models/<model_summary>
  : cc-workon processes/<process_name>
  ```
  这个命令会通过 hook 将当前 session 与目标项目目录绑定。**每轮对话结束时 Stop hook 自动清除绑定**（`hooks/session_cleanup.py`），因此新一轮需要 MCP 写入工具时必须重新执行 cc-workon。这是预期行为，防止跨 turn 复用绑定导致 spec gate 绕过。详细规则：
